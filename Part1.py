...
# hidden_layers = 10
# grid = []
# for hidden_layers in [10]:
#     nbr_hidden_layers = [[10]] #[[1],[2],[3]][hidden_layers-1]
#     nbr_hidden_neurons_per_layer = [[250, 200, 150, 125, 100, 75, 50, 25, 10, 5]] #Need to be a list of lists (not tuple which is immutable and we want to modify this later)
#     hidden_activation_function_per_layer = ['relu','relu','relu','relu','relu','relu','relu','relu','relu','relu'] #Needs to be a list of lists
#     optimizer = ['Adam']
...
def build_model(nbr_hidden_layers = 3, 
        ...
        if optimizer == 'SGD':
            opt = tf.keras.optimizers.SGD(lr = learning_rate, clipnorm = 1.0)
            ...
            learning_schedule= tf.keras.optimizers.schedules.ExponentialDecay(
                ...
                decay_steps, 
                decay_rate)
            opt = tf.keras.optimizers.SGD(learning_rate=learning_schedule, clipnorm = 1.0)
...
for grid_record in grid[:1]:
    ...
    out = build_model(nbr_hidden_layers = 10, 
                      nbr_hidden_neurons_per_layer = [250, 200, 150, 125, 100, 75, 50, 25, 10, 5],
                      hidden_activation_function_per_layer = ['relu','relu','relu','relu','relu','relu','relu','relu','relu','relu'],
                      optimizer = 'Adam',
                      learning_rate = 0.0001,
                      batch_size = 1,
                      timestep = nbr_of_time_steps) 
    model = out[0]
    batch_size = out[1]    
    ...
    for epoch in range(epochs):
        ...
        for chunk in reader:
            ...
            ## CHANGE NUMBER OF RECORDS THE MODEL ITERATES THROUGH
            if i >= iter_through:
                break
